export function buildNetworkActions(self) {
    return {
        ...
        network_create_network: {
            ...
            description: 'Creates a new network configuration (IP subnet, VLAN, and routing settings) in the UniFi Controller.',
            options: [
        ...
        network_delete_network: {
            ...
            description: 'Permanently deletes a network configuration and all its associated settings from the UniFi Controller.',
            options: [
        ...
        network_list_portconf: {
            ...
            description: 'Retrieves port configuration profiles (VLAN assignments, PoE settings, etc.) for switches from the UniFi Controller.',
            options: [],
            callback: async () => {
                // Every action bails out early if the module isn't currently connected to the console
                const unifi = self.unifi;
        ...
        network_create_dns_record: {
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const record_typeValue = event.options.record_type;
                    // Run the command against the UniFi console
                    const result = await unifi
                        .getNetworkManagementAPI()
                        .create_dns_record(record_typeValue, event.options.value, event.options.key, event.options.ttl, event.options.enabled, self.config.site);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
        ...
        network_create_dynamicdns: {
            ...
            description: 'Creates a new dynamic DNS configuration that automatically updates DNS records when the IP address changes.',
            options: [
                // unifi-api-ts parameter "payload" (required) — a raw JSON payload, typed here as text
                {
                    id: 'payload',
        ...
        network_create_wlan: {
            ...
            description: 'Creates a new wireless network (WLAN/SSID) with the specified security settings, user group, and advanced configuration options.',
            options: [
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        .getNetworkManagementAPI()
                        .create_wlan(event.options.name, event.options.x_passphrase, event.options.usergroup_id, event.options.wlangroup_id, event.options.enabled, event.options.hide_ssid, event.options.is_guest, event.options.security, event.options.wpa_mode, event.options.wpa_enc, event.options.vlan_enabled, event.options.vlan_id, event.options.uapsd_enabled, event.options.schedule_enabled, schedulePayload, ap_group_idsList, additional_payloadPayload, self.config.site);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
        ...
        network_list_wlanconf: {
            ...
            description: 'Retrieves all WLAN (wireless network) configurations, including SSIDs, security settings, and user groups.',
            options: [],
            callback: async () => {
                // Every action bails out early if the module isn't currently connected to the console
                const unifi = self.unifi;
        ...
        network_list_country_codes: {
            ...
            description: 'Retrieves the supported country codes used for wireless regulatory compliance (allowed channels and power levels).',
            options: [],
            callback: async () => {
                // Every action bails out early if the module isn't currently connected to the console
                const unifi = self.unifi;
        ...
        network_list_current_channels: {
            ...
            description: 'Retrieves the wireless channels currently in use across all Access Points, for channel planning and interference analysis.',
            options: [],
            callback: async () => {
                // Every action bails out early if the module isn't currently connected to the console
                const unifi = self.unifi;
        ...
        network_spectrum_scan: {
            ...
            description: 'Initiates a spectrum scan on a specific Access Point to analyze wireless interference and channel utilization.',
            options: [
