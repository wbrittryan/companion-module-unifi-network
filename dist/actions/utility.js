...
const DEVICE_COMMAND_REQUIRES_TYPE = {
    'power-cycle': 'usw',
};
export function buildUtilityActions(self) {
    return {
        ...
        utility_send_device_command: {
            ...
            options: [
                ...
                },
                {
                    ...
                    type: 'dropdown',
                    label: 'Device',
                    tooltip: "Pick a device from the list (pulled live from the console) — or type a MAC address directly, e.g. aa:bb:cc:dd:ee:ff, if it isn't in the list yet.",
                ...
                },
                {
                    ...
                    tooltip: 'What to do to the device selected above. "Power Cycle Switch Port" only works on switches, and "Set Outlet Power" only works on a UniFi SmartPower device (UPS, UPS Pro, PDU Pro, Smart Plug, Smart Strip, etc) — picking either for a device that doesn\'t support it is rejected before anything is sent.',
                    choices: [
                ...
                },
                {
                    ...
                    type: 'dropdown',
                    label: 'Reboot Type',
                    tooltip: "'soft' is a normal graceful reboot; 'hard' forces a rougher reboot for a device that's unresponsive.",
                    choices: [
                ...
                try {
                    ...
                    if (targetType === 'device') {
                        ...
                        if (requiredType && device && device.type !== requiredType) {
                            throw new Error(`"${device.name ?? mac}" is a ${DEVICE_TYPE_LABELS[device.type] ?? device.type}, not a ${DEVICE_TYPE_LABELS[requiredType] ?? requiredType} — this command doesn't apply to it`);
                        }
                        ...
                        if (command === 'set-outlet' && device) {
                            const cachedOutlets = device.outlet_table;
                            if (!cachedOutlets || cachedOutlets.length === 0) {
                                throw new Error(`"${device.name ?? mac}" doesn't report any controllable outlets — Set Outlet Power only works on UniFi SmartPower devices (UPS, UPS Pro, PDU Pro, Smart Plug, Smart Strip, etc)`);
                            }
                        }
                    }
                    ...
                    if (command === 'set-outlet') {
                        ...
                        const deviceWithOutlets = freshDevice;
                        ...
                        if (!deviceWithOutlets.outlet_table || deviceWithOutlets.outlet_table.length === 0) {
                            throw new Error(`"${deviceWithOutlets.name ?? mac}" doesn't report any controllable outlets — Set Outlet Power only works on UniFi SmartPower devices (UPS, UPS Pro, PDU Pro, Smart Plug, Smart Strip, etc)`);
                        }
                        ...
                        if (command === 'authorize-guest')
                            payload.minutes = event.options.minutes;
                        if (command === 'restart')
                            payload.reboot_type = event.options.reboot_type;
                        if (command === 'power-cycle')
                            payload.port_idx = event.options.port_idx;
