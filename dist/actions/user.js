export function buildUserActions(self) {
    return {
        ...
        user_create_user: {
            ...
            description: 'Creates a new user entry for a client device with a specified MAC address and assigns it to a user group for bandwidth control and access policies.',
            options: [
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        ...
                        .create_user(self.config.site, event.options.mac, event.options.user_group_id, event.options.name, event.options.note, event.options.is_guest, event.options.is_wired);
        ...
        user_set_usergroup: {
            ...
            description: 'Assigns a client device to a specific user group, controlling bandwidth limits and access policies for that device.',
            options: [
        ...
        user_create_usergroup: {
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        .getUserManagementAPI()
                        .create_usergroup(self.config.site, event.options.group_name, event.options.group_dn, event.options.group_up);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
        ...
        user_list_usergroups: {
            ...
            description: 'Retrieves all user groups configured in the site, which define bandwidth limits and access policies for client devices.',
            options: [],
            callback: async () => {
                // Every action bails out early if the module isn't currently connected to the console
                const unifi = self.unifi;
        ...
        user_delete_usergroup: {
            ...
            description: 'Removes a user group from the site; users assigned to the deleted group are moved to the default user group.',
            options: [
                // unifi-api-ts parameter "group_id" (required)
                {
                    id: 'group_id',
        ...
        user_list_admins: {
            ...
            description: 'Retrieves a list of administrators who have access to the current site, including their roles and permissions.',
            options: [],
            callback: async () => {
                // Every action bails out early if the module isn't currently connected to the console
                const unifi = self.unifi;
        ...
        user_invite_admin: {
            ...
            description: 'Invites a new administrator to the site with specified permissions; the invitee receives an email invitation to access the UniFi Controller.',
            options: [
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        .getUserManagementAPI()
                        .invite_admin(self.config.site, event.options.name, event.options.email, event.options.enable_sso, event.options.readonly, event.options.device_adopt, event.options.device_restart);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
        ...
        user_assign_existing_admin: {
            ...
            description: 'Grants an existing administrator access to a specific site with configurable permissions and roles.',
            options: [
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        .getUserManagementAPI()
                        .assign_existing_admin(self.config.site, event.options.admin_id, event.options.readonly, event.options.device_adopt, event.options.device_restart);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
        ...
        user_update_admin: {
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        .getUserManagementAPI()
                        .update_admin(self.config.site, event.options.admin_id, event.options.name, event.options.email, event.options.password, event.options.readonly, event.options.device_adopt, event.options.device_restart, event.options.is_super);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
        ...
        user_grant_super_admin: {
            ...
            description: 'Grants super administrator privileges to an existing administrator, giving them elevated permissions across the entire UniFi Controller.',
            options: [
                // unifi-api-ts parameter "admin_id" (required)
                {
                    id: 'admin_id',
        ...
        user_delete_admin: {
            ...
            description: 'Removes an administrator from the site by revoking their super admin privileges (equivalent to revoke_admin but using the delete-admin command).',
            options: [
                // unifi-api-ts parameter "admin_id" (required)
                {
                    id: 'admin_id',
        ...
        user_edit_usergroup: {
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const result = await unifi
                        .getUserManagementAPI()
                        .edit_usergroup(self.config.site, event.options.group_id, event.options.site_id, event.options.group_name, event.options.group_dn, event.options.group_up);
                    // Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
