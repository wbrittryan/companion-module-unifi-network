...
export function UpdateActions(self) {
    self.setActionDefinitions({
        ...
        device_led_override: {
            ...
            options: [
                ...
                // unifi-api-ts parameter "override_mode" (required) — one of: off, on, default
                {
                    ...
                    type: 'dropdown',
                    label: 'Override Mode',
                    tooltip: "Type exactly 'off' to force the LED off, 'on' to force it on, or 'default' to let the controller control it normally.",
                    choices: [
                        { id: 'off', label: 'off' },
                        { id: 'on', label: 'on' },
            ...
            callback: async (event) => {
                ...
                try {
                    ...
                    const overrideModeValue = event.options.override_mode;
        ...
        device_locate_ap: {
            ...
            description: "Use this to make an access point's LED blink so you can physically find it in a rack, ceiling, or storage room — press it again with enable turned off to stop the blinking. This has no effect on Wi-Fi service; it's purely a 'find the box' helper.",
            options: [
                // unifi-api-ts parameter "mac" (required)
                {
                    id: 'mac',
        ...
        device_restart_device: {
            ...
            options: [
                ...
                // unifi-api-ts parameter "reboot_type" (optional) — one of: soft, hard
                {
                    ...
                    type: 'dropdown',
                    label: 'Reboot Type',
                    tooltip: "Type 'soft' for a normal graceful reboot (default), or 'hard' to force a harder reboot for an unresponsive device.",
                    choices: [
                        { id: 'soft', label: 'soft' },
                        { id: 'hard', label: 'hard' },
                    ],
        ...
        stats_archive_alarm: {
            ...
            options: [
                ...
                // something you'd typically know ahead of time anyway (this is normally left blank).
                {
                    ...
                    type: 'textinput',
                    label: 'Alarm ID',
                    tooltip: 'The ID of one specific alarm to dismiss, e.g. 507f1f77bcf86cd799439011. Leave blank to dismiss (archive) all current alarms at once.',
                    default: '',
                },
            ],
            callback: async (event) => {
