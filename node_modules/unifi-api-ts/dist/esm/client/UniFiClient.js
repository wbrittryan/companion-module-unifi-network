 ...
 * @since 1.0.0
 */
import { HTTPClient, SessionManager } from '../http/index.js';
import { ConfigurationError } from '../errors/index.js';
import { AuthenticationAPI } from '../api/authentication/index.js';
import { DeviceManagementAPI } from '../api/device-management/index.js';
import { ClientManagementAPI } from '../api/client-management/index.js';
import { NetworkManagementAPI } from '../api/network-management/index.js';
import { SiteManagementAPI } from '../api/site-management/index.js';
import { UserManagementAPI } from '../api/user-management/index.js';
...
export class UniFiClient {
    ...
    // Core API methods
    async listUsers(clientMac, options) {
        // If first parameter is actually options object, handle it
        if (clientMac && typeof clientMac === 'object' && 'signal' in clientMac) {
            return this.clientManagementAPI.list_users(this.config.site || 'default', clientMac);
        }
        return this.clientManagementAPI.list_users(this.config.site || 'default', options);
    }
    async listDevices(macs, options) { return this.deviceManagementAPI.list_devices(macs, options); }
    async statSysinfo(options) { return this.statisticsAPI.stat_sysinfo(this.config.site || 'default', options); }
    // Device management
    async adoptDevice(macs, options) {
    ...
    async advancedAdoptDevice(mac, ip, username, password, url, port, sshKeyVerify, options) {
        return this.deviceManagementAPI.advanced_adopt_device(mac, ip, username, password, url, port, sshKeyVerify, options);
    }
    async blockSta(mac, options) { return this.clientManagementAPI.block_sta(this.config.site || 'default', mac, options); }
    async forceProvision(macs, options) { return this.deviceManagementAPI.force_provision(macs, options); }
    async forgetSta(macs, options) { return this.clientManagementAPI.forget_sta(this.config.site || 'default', macs, options); }
    // Guest management
    async authorizeGuest(mac, minutes, up, down, megabytes, apMac, options) {
        return this.clientManagementAPI.authorize_guest(this.config.site || 'default', mac, minutes, up, down, megabytes, apMac, options);
    }
    async extendGuestValidity(guestId, options) { return this.clientManagementAPI.extend_guest_validity(this.config.site || 'default', guestId, options); }
    // WLAN management
    async createWlan(name, xPassphrase, usergroupId, wlangroupId, enabled, hideSsid, isGuest, security, wpaMode, wpaEnc, vlanEnabled, vlanId, uapsdEnabled, scheduleEnabled, schedule, apGroupIds, additionalPayload, options) {
    ...
    async deleteWlan(wlanId, options) { return this.networkManagementAPI.delete_wlan(wlanId, this.config.site || 'default', options); }
    // Network management
    async createNetwork(payload, options) { return this.networkManagementAPI.create_network(payload, this.config.site || 'default', options); }
    async deleteNetwork(networkId, options) { return this.networkManagementAPI.delete_network(networkId, this.config.site || 'default', options); }
    async listNetworks(networkId, options) { return this.networkManagementAPI.list_networkconf(networkId, this.config.site || 'default', options); }
    async updateNetworkSettings(networkId, payload, options) { return this.networkManagementAPI.set_networksettings_base(networkId, payload, this.config.site || 'default', options); }
    async listRouting(routeId, options) { return this.networkManagementAPI.list_routing(routeId, this.config.site || 'default', options); }
    async listPortConfigurations(options) { return this.networkManagementAPI.list_portconf(this.config.site || 'default', options); }
    async listPortForwarding(options) { return this.networkManagementAPI.list_portforwarding(this.config.site || 'default', options); }
    async listPortForwardingStats(options) { return this.networkManagementAPI.list_portforward_stats(this.config.site || 'default', options); }
    async createDnsRecord(recordType, value, key, ttl, enabled, options) {
        return this.networkManagementAPI.create_dns_record(recordType, value, key, ttl, enabled, this.config.site || 'default', options);
    }
    async listDnsRecords(options) { return this.networkManagementAPI.list_dns_records(this.config.site || 'default', options); }
    async deleteDnsRecord(recordId, options) { return this.networkManagementAPI.delete_dns_record(recordId, this.config.site || 'default', options); }
    async createDynamicDns(payload, options) { return this.networkManagementAPI.create_dynamicdns(payload, this.config.site || 'default', options); }
    async listDynamicDns(options) { return this.networkManagementAPI.list_dynamicdns(this.config.site || 'default', options); }
    async updateDynamicDns(dynamicDnsId, payload, options) { return this.networkManagementAPI.set_dynamicdns(dynamicDnsId, payload, this.config.site || 'default', options); }
    // Site management
    async createSite(description, options) {
        // Validate required parameters
    ...
    async deleteSite(siteId, options) { return this.siteManagementAPI.delete_site(siteId, options); }
    // Backup management
    async generateBackup(options) { return this.siteManagementAPI.generate_backup(options); }
    async generateBackupSite(options) { return this.siteManagementAPI.generate_backup_site(options); }
    async downloadBackup(filepath, options) { return this.siteManagementAPI.download_backup(filepath, options); }
    async listBackups(options) { return this.siteManagementAPI.list_backups(options); }
    // System management
    async updateOsConsole(options) { return this.siteManagementAPI.update_os_console(options); }
    async getUpdateOsConsole(options) { return this.siteManagementAPI.get_update_os_console(options); }
    // Settings management
    async listSettings(options) { return this.siteManagementAPI.list_settings(options); }
    async listSelf(options) { return this.siteManagementAPI.list_self(options); }
    async setSuperIdentitySettingsBase(payload, options) { return this.siteManagementAPI.set_super_identity_settings_base(payload, options); }
    async setSuperMgmtSettingsBase(payload, options) { return this.siteManagementAPI.set_super_mgmt_settings_base(payload, options); }
    async setSuperSmtpSettingsBase(payload, options) { return this.siteManagementAPI.set_super_smtp_settings_base(payload, options); }
    // Backup & System management (underscore naming for backward compatibility)
    async download_backup(filepath, options) { return this.siteManagementAPI.download_backup(filepath, options); }
    async generate_backup(options) { return this.siteManagementAPI.generate_backup(options); }
    async generate_backup_site(options) { return this.siteManagementAPI.generate_backup_site(options); }
    async list_backups(options) { return this.siteManagementAPI.list_backups(options); }
    async update_os_console(options) { return this.siteManagementAPI.update_os_console(options); }
    async get_update_os_console(options) { return this.siteManagementAPI.get_update_os_console(options); }
    // Settings management (underscore naming for backward compatibility)
    async list_settings(options) { return this.siteManagementAPI.list_settings(options); }
    async list_self(options) { return this.siteManagementAPI.list_self(options); }
    async set_super_identity_settings_base(payload, options) { return this.siteManagementAPI.set_super_identity_settings_base(payload, options); }
    async set_super_mgmt_settings_base(payload, options) { return this.siteManagementAPI.set_super_mgmt_settings_base(payload, options); }
    async set_super_smtp_settings_base(payload, options) { return this.siteManagementAPI.set_super_smtp_settings_base(payload, options); }
    // Alarm management
    async archiveAlarm(alarmId, options) { return this.statisticsAPI.archive_alarm(alarmId, this.config.site, options); }
    async countAlarms(archived, options) { return this.statisticsAPI.count_alarms(archived, this.config.site, options); }
    // User management
    async createUser(mac, userGroupId, name, note, isGuest, isWired, options) {
        return this.userManagementAPI.create_user(this.config.site || 'default', mac, userGroupId, name, note, isGuest, isWired, options);
    }
    async createUsergroup(groupName, groupDn, groupUp, options) {
        return this.userManagementAPI.create_usergroup(this.config.site || 'default', groupName, groupDn || -1, groupUp || -1, options);
    }
    async deleteUsergroup(groupId, options) { return this.userManagementAPI.delete_usergroup(groupId, this.config.site || 'default', options); }
    // Voucher management
    async createVoucher(minutes, count, quota, note, up, down, megabytes, options) {
    ...
    // Tag management
    async createTag(name, macs, options) { return this.clientManagementAPI.create_tag(name, macs, this.config.site || 'default', options); }
    async deleteTag(tagId, options) { return this.clientManagementAPI.delete_tag(tagId, this.config.site || 'default', options); }
    // Access point group management
    async createApgroup(groupName, deviceMacs, options) {
        return this.deviceManagementAPI.create_apgroup(groupName, deviceMacs, options);
    }
    async deleteApgroup(groupId, options) {
        return this.deviceManagementAPI.delete_apgroup(groupId, options);
    }
    async editApgroup(groupId, groupName, deviceMacs, options) {
    ...
    async create_apgroup(groupName, deviceMacs = [], options) {
        return this.deviceManagementAPI.create_apgroup(groupName, deviceMacs, options);
    }
    async delete_apgroup(groupId, options) {
        return this.deviceManagementAPI.delete_apgroup(groupId, options);
    }
    async edit_apgroup(groupId, groupName, deviceMacs, options) {
        return this.deviceManagementAPI.edit_apgroup(groupId, groupName, deviceMacs, options);
    }
    // Firewall group management
    async createFirewallgroup(groupName, groupType, groupMembers, options) {
        return this.securityAPI.create_firewallgroup(groupName, groupType, groupMembers, this.config.site || 'default', options);
    }
    async deleteFirewallgroup(groupId, options) { return this.securityAPI.delete_firewallgroup(groupId, this.config.site || 'default', options); }
    async editFirewallgroup(groupId, siteId, groupName, groupType, groupMembers, options) {
    ...
    async createRadiusAccount(name, xPassword, tunnelType, tunnelMediumType, vlan, options) {
    ...
    async deleteRadiusAccount(accountId, options) { return this.securityAPI.delete_radius_account(accountId, this.config.site || 'default', options); }
    // Dynamic DNS management
    async createDynamicdns(payload, options) { return this.networkManagementAPI.create_dynamicdns(payload, this.config.site || 'default', options); }
    // Hotspot operator management
    async createHotspotop(name, xPassword, note, options) {
        return this.clientManagementAPI.create_hotspotop(name, xPassword, note, this.config.site || 'default', options);
    }
    // Device management
    async cancelMigrateDevice(macs, options) { return this.deviceManagementAPI.cancel_migrate_device(macs, options); }
    async cancelRollingUpgrade(options) { return this.deviceManagementAPI.cancel_rolling_upgrade(options); }
    async checkControllerUpdate(options) {
        return this.statisticsAPI.check_controller_update(this.config.site || 'default', options);
    }
    async checkFirmwareUpdate(options) { return this.deviceManagementAPI.check_firmware_update(options); }
    async deleteDevice(mac, options) { return this.deviceManagementAPI.delete_device(mac, options); }
    async disableAp(apId, disable, options) { return this.deviceManagementAPI.disable_ap(apId, disable, options); }
    async disableWlan(wlanId, disable, options) {
        return this.networkManagementAPI.disable_wlan(wlanId, disable, this.config.site || 'default', options);
    }
    // Client management
    async editClientFixedip(clientId, useFixedip, networkId, fixedIp, options) {
        return this.clientManagementAPI.edit_client_fixedip(this.config.site, clientId, useFixedip, networkId, fixedIp, options);
    }
    async editClientName(clientId, name, options) {
        return this.clientManagementAPI.edit_client_name(this.config.site, clientId, name, options);
    }
    async editUsergroup(groupId, siteId, groupName, groupDn, groupUp, options) {
        return this.userManagementAPI.edit_usergroup(this.config.site || 'default', groupId, siteId, groupName, groupDn, groupUp, options);
    }
    // Admin management
    async assignExistingAdmin(adminId, readonly, deviceAdopt, deviceRestart, options) {
    ...
    async deleteAdmin(adminId, options) { return this.userManagementAPI.delete_admin(this.config.site || 'default', adminId, options); }
    // Statistics and command methods
    async cmdStat(command, options) { return this.statisticsAPI.cmd_stat(command, this.config.site || 'default', options); }
    // Custom API request
    async customApiRequest(path, method, payload, returnType, options) {
    ...
    getClassVersion() { return '1.0.0'; }
    getConnectionTimeout() { return 30000; } // Default timeout
    getCookie() { return ''; } // Placeholder
    getCookies() { return ''; } // Placeholder
    getCookiesCreatedAt() { return Date.now(); } // Placeholder
    getCurlHttpVersion() { return '2.0'; }
    getCurlMethod() { return 'POST'; }
    getCurlRequestTimeout() { return 30000; } // Default timeout
    getCurlSslVerifyHost() { return 2; } // Default verify
    getCurlSslVerifyPeer() { return true; } // Default verify
    // ============================================================================
    // BATCH 6 METHODS (methods 76-90)
    // ============================================================================
    // List methods from batch 6
    async listAllAdmins(options) { return this.userManagementAPI.list_all_admins(options); }
    async listApgroups(options) { return this.deviceManagementAPI.list_apgroups(options); }
    async listAps(mac, options) { return this.deviceManagementAPI.list_aps(mac, options); }
    async listClients(mac, options) { return this.clientManagementAPI.list_clients(this.config.site, mac, options); }
    async listClientsHistory(historyhours, start, end, mac, options) {
        return this.clientManagementAPI.list_clients_history(this.config.site, historyhours, start, end, mac, options);
    }
    async listCountryCodes(options) { return this.networkManagementAPI.list_country_codes(this.config.site, options); }
    async listCurrentChannels(options) { return this.networkManagementAPI.list_current_channels(this.config.site, options); }
    async listDashboard(fiveMinutes, options) { return this.statisticsAPI.list_dashboard(fiveMinutes, this.config.site, options); }
    async listDeviceNameMappings(options) { return this.deviceManagementAPI.list_device_name_mappings(options); }
    async listDeviceStates(options) { return this.deviceManagementAPI.list_device_states(options); }
    async listDevicesBasic(mac, options) { return this.deviceManagementAPI.list_devices_basic(mac, options); }
    async listDpiStats(options) { return this.statisticsAPI.list_dpi_stats(this.config.site, options); }
    async listDpiStatsFiltered(type, catFilter, options) {
        return this.statisticsAPI.list_dpi_stats_filtered(type, catFilter, this.config.site, options);
    }
    // ============================================================================
    // BATCH 7 METHODS (methods 91-105)
    // ============================================================================
    async listDynamicdns(options) {
        return this.networkManagementAPI.list_dynamicdns(this.config.site, options);
    }
    async listExtension(options) {
        return this.statisticsAPI.list_extension(this.config.site, options);
    }
    async listFingerprintDevices(options) {
        return this.statisticsAPI.list_fingerprint_devices(this.config.site, options);
    }
    async listFirewallgroups(options) {
        return this.securityAPI.list_firewallgroups(this.config.site, options);
    }
    async listFirewallrules(options) {
        return this.securityAPI.list_firewallrules(this.config.site, options);
    }
    async listFirmware(type, options) {
        return this.deviceManagementAPI.list_firmware(type, options);
    }
    async listGuests(within, options) {
        return this.clientManagementAPI.list_guests(this.config.site, within, options);
    }
    async listHealth(options) {
        return this.statisticsAPI.list_health(this.config.site, options);
    }
    async listHotspotop(options) {
        return this.clientManagementAPI.list_hotspotop(this.config.site, options);
    }
    async listKnownRogueaps(options) {
    ...
    async delete_admin(admin_id, options) {
        return this.userManagementAPI.delete_admin(this.config.site || 'default', admin_id, options);
    }
    async edit_usergroup(group_id, site_id, group_name, group_dn = -1, group_up = -1, options) {
    ...
    async list_devices(macs, options) {
        return this.deviceManagementAPI.list_devices(macs, options);
    }
    async adopt_device(macs, options) {
        return this.deviceManagementAPI.adopt_device(macs, options);
    }
    async restart_device(macs, reboot_type = 'soft', options) {
        return this.deviceManagementAPI.restart_device(macs, reboot_type, options);
    }
    async list_users(client_mac, options) {
        return this.clientManagementAPI.list_users(this.config.site, options);
    }
    async block_sta(mac, options) {
        return this.clientManagementAPI.block_sta(this.config.site, mac, options);
    }
    async authorize_guest(mac, minutes, up, down, megabytes, apMac, options) {
        return this.clientManagementAPI.authorize_guest(this.config.site, mac, minutes, up, down, megabytes, apMac, options);
    }
    async list_wlanconf(wlan_id, options) {
        return this.networkManagementAPI.list_wlanconf(this.config.site, options);
    }
    async create_wlan(name, x_passphrase, usergroup_id, wlangroup_id, enabled, hide_ssid, is_guest, security, wpa_mode, wpa_enc, vlan_enabled, vlan_id, uapsd_enabled, schedule_enabled, schedule, ap_group_ids, additional_payload, options) {
