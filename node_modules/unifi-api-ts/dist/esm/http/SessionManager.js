 ...
 * ```
 */
import { AuthenticationError, SessionExpiredError, ValidationUtils, ConfigurationError } from '../errors/index.js';
export class SessionManager {
    ...
    async performLogin() {
        try {
                ...
                try {
                    ...
                    if (isNetworkControllerFormat) {
                        ...
                        if (responseData.meta?.rc !== 'ok') {
                            throw new AuthenticationError(responseData.meta?.msg || 'Login failed', response);
                        }
                    }
                        ...
                        // Unknown format
                        throw new AuthenticationError('Unknown response format from UniFi controller', response);
                    }
                    // Update session info
                    this.sessionInfo = {
            ...
            throw new AuthenticationError(`Login failed: ${errorMessage}`, lastError);
        }
        catch (error) {
            this.sessionInfo = { isAuthenticated: false };
            if (error instanceof AuthenticationError) {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new AuthenticationError(`Login failed: ${errorMessage}`, error);
        }
    }
    /**
