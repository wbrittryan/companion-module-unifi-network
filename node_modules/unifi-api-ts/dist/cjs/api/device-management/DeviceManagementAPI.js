...
exports.DeviceManagementAPI = void 0;
class DeviceManagementAPI {
     ...
     */
    async advanced_adopt_device(mac, ip, username, password, url, port = 22, ssh_key_verify = true, options) {
        const payload = {
            cmd: 'adv-adopt',
            mac: mac.toLowerCase(),
            ip,
    ...
    /**
      ...
      */
    async cancel_migrate_device(macs, options) {
        const mac_array = Array.isArray(macs) ? macs : [macs];
        const payload = {
            macs: mac_array.map(mac => mac.toLowerCase()),
            cmd: 'cancel-migrate'
     ...
     */
    async cancel_rolling_upgrade(options) {
        const payload = {
     ...
     */
    async check_firmware_update(options) {
        const payload = {
     ...
     */
    async delete_device(mac, options) {
        const payload = {
            cmd: 'delete-device',
            mac: mac.toLowerCase()
        };
     ...
     */
    async disable_ap(ap_id, disable, options) {
        const payload = {
            disabled: disable
        };
        return await this.makeRequest({
     ...
     */
    async force_provision(macs, options) {
        const mac_array = Array.isArray(macs) ? macs : [macs];
        if (mac_array.length === 0) {
...
*
     ...
     */
    async led_override(device_id, override_mode, options) {
        const payload = {
     ...
     */
    async list_device_name_mappings(options) {
        return await this.makeRequest({
            method: 'GET',
     ...
     */
    async list_device_states(options) {
        return await this.makeRequest({
            method: 'GET',
     ...
     */
    async list_devices_basic(device_mac, options) {
        if (device_mac) {
            const payload = {
     ...
     */
    async list_firmware(device_type, options) {
        if (device_type) {
            const payload = {
                type: device_type
            };
    ...
    /**
      ...
      */
    async locate_ap(mac, enable, options) {
     ...
     */
    async migrate_device(macs, inform_url, options) {
        const mac_array = Array.isArray(macs) ? macs : [macs];
     ...
     */
    async power_cycle_switch_port(mac, port_idx, options) {
        if (!mac || typeof mac !== 'string') {
            throw new Error('MAC address is required and must be a string');
        }
     ...
     */
    async rename_ap(ap_id, ap_name, options) {
     ...
     */
    async set_ap_radiosettings(ap_id, radio, channel, ht, tx_power_mode, tx_power, options) {
        const payload = {
            radio_table: [{
                    radio,
                    channel,
     ...
     */
    async set_ap_wlangroup(type_id, device_id, group_id, options) {
        const payload = {
            wlan_overrides: [{
                    radio: type_id,
     ...
     */
    async set_device_settings_base(device_id, payload, options) {
     ...
     */
    async set_element_adoption(enable, options) {
        const payload = {
            enabled: enable
        };
        return await this.makeRequest({
     ...
     */
    async start_rolling_upgrade(payload = ['uap', 'usw', 'ugw', 'uxg'], options) {
        return await this.makeRequest({
            method: 'POST',
     ...
     */
    async upgrade_all_devices(type = 'uap', options) {
        const payload = {
            type
        };
        return await this.makeRequest({
     ...
     */
    async upgrade_device(mac, options) {
        if (!mac || typeof mac !== 'string') {
            throw new Error('MAC address is required and must be a string');
        }
        const payload = {
     ...
     */
    async upgrade_device_external(firmware_url, macs, options) {
     ...
     */
    async create_apgroup(group_name, device_macs = [], options) {
     ...
     */
    async delete_apgroup(group_id, options) {
     ...
     */
    async edit_apgroup(group_id, group_name, device_macs, options) {
        if (!group_id || typeof group_id !== 'string') {
            throw new Error('Group ID is required and must be a string');
        }
        if (!group_name || typeof group_name !== 'string') {
...
exports.DeviceManagementAPI = DeviceManagementAPI;
//# sourceMappingURL=DeviceManagementAPI.js.map