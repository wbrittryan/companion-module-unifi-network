...
const axiosCookieJarSupport = __importStar(require("axios-cookiejar-support"));
...
class HTTPClient {
    ...
    constructor(config) {
        ...
        this._verifySsl = config.verifySsl !== false;
        ...
        if (config.verifySsl === false) {
            ...
            this._originalTlsRejectUnauthorized = process.env.NODE_TLS_REJECT_UNAUTHORIZED;
    ...
    setupInterceptors() {
        ...
        this.client.interceptors.request.use((config) => {
            ...
            var _a;
            if (this.debug) {
                console.log(`[HTTP] ${(_a = config.method) === null || _a === void 0 ? void 0 : _a.toUpperCase()} ${config.url}`);
                if (config.data) {
    ...
    handleError(error) {
        ...
        if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') {
            return new errors_1.TimeoutError('Request timeout');
        }
        // Handle network errors
        if (!error.response) {
            return errors_1.ErrorFactory.fromNetworkError(error);
        }
        ...
        return errors_1.ErrorFactory.fromHttpStatus(status, message, data);
    }
    /**
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                if (response.data && response.data.meta && response.data.meta.rc === 'error') {
                    throw new errors_1.APIError(response.data.meta.msg || 'API Error', 400, response.data);
                }
                ...
                if (error instanceof errors_1.APIError && error.statusCode && error.statusCode < 500) {
                    throw error;
                }
                ...
                if ((_b = config.signal) === null || _b === void 0 ? void 0 : _b.aborted) {
                    throw new Error('Request was cancelled');
                }
     ...
     */
    restoreSSLSettings() {
        const originalValue = this._originalTlsRejectUnauthorized;
