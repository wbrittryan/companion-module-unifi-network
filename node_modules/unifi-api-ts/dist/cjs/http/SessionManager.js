...
class SessionManager {
    ...
    validateConfig() {
        try {
            errors_1.ValidationUtils.validateUrl(this.config.baseUrl, 'baseUrl');
            errors_1.ValidationUtils.validateString(this.config.username, 'username', 1);
            errors_1.ValidationUtils.validateString(this.config.password, 'password', 1);
            if (this.config.site) {
                errors_1.ValidationUtils.validateString(this.config.site, 'site', 1);
            }
            if (this.config.timeout) {
                errors_1.ValidationUtils.validateNumber(this.config.timeout, 'timeout', 1000);
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new errors_1.ConfigurationError(`Invalid configuration: ${errorMessage}`);
        }
    }
    /**
    ...
    async performLogin() {
        ...
        try {
            ...
            for (const endpoint of loginEndpoints) {
                try {
                    ...
                    if (isNetworkControllerFormat) {
                        ...
                        if (((_a = responseData.meta) === null || _a === void 0 ? void 0 : _a.rc) !== 'ok') {
                            throw new errors_1.AuthenticationError(((_b = responseData.meta) === null || _b === void 0 ? void 0 : _b.msg) || 'Login failed', response);
                        }
                    }
                        ...
                        // Unknown format
                        throw new errors_1.AuthenticationError('Unknown response format from UniFi controller', response);
                    }
                    // Update session info
                    this.sessionInfo = {
            ...
            throw new errors_1.AuthenticationError(`Login failed: ${errorMessage}`, lastError);
        }
        catch (error) {
            this.sessionInfo = { isAuthenticated: false };
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new errors_1.AuthenticationError(`Login failed: ${errorMessage}`, error);
        }
    }
    /**
    ...
    async refreshSession() {
        try {
            ...
            if (error instanceof errors_1.AuthenticationError) {
                this.sessionInfo = { isAuthenticated: false };
                throw new errors_1.SessionExpiredError('Session has expired');
            }
