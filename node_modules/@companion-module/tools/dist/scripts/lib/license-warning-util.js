...
const parse = __require("spdx-expression-parse");
...
export const SUPPORTED_PROJECT_LICENSES = Object.keys(PROJECT_LICENSE_POLICIES);
...
const LICENSE_HELP_MESSAGE = 'Not sure what to do about these? Ask in the Bitfocus community Slack, we are happy to help you work out what they mean for your module.';
function isProjectLicense(declaration) {
    return SUPPORTED_PROJECT_LICENSES.includes(declaration);
}
...
function parseLenient(declaration) {
    ...
    function parseFactor() {
        ...
        if (token === '(') {
            ...
            if (!node || tokens[index] !== ')')
                return undefined;
            index++;
            return node;
        }
        if (keyword(token, 'AND') || keyword(token, 'OR') || keyword(token, 'WITH'))
            return undefined;
        index++;
        const plus = token.endsWith('+');
        const node = { license: plus ? token.slice(0, -1) : token };
        if (plus)
            node.plus = true;
...
function createSourceLicenseIssue(packageInfo) {
    ...
    if (sourceLicense === REQUIRED_SOURCE_LICENSE)
        return undefined;
    if (!sourceLicense) {
        return issue(packageInfo, `Your module source does not declare a license in package.json, but the Companion project requires module source to be ${REQUIRED_SOURCE_LICENSE} so it stays portable.`);
    }
    return issue(packageInfo, `Your module source is licensed as ${displayDeclaration(sourceLicense)} in package.json, but the Companion project requires module source to be ${REQUIRED_SOURCE_LICENSE} so it stays portable. Declare the license the module is distributed under in companion/manifest.json instead.`);
}
function createDistributionLicenseIssue(packageInfo) {
    const declaration = normalizedDeclaration(packageInfo);
    if (!declaration) {
        return issue(packageInfo, `Your module does not declare a license in companion/manifest.json. ${supportedProjectLicenseAdvice()} Talk to us if you have a reason to use a different license.`);
    }
    if (isProjectLicense(declaration))
        return undefined;
    ...
    try {
        ...
        if (!('license' in parse(declaration)))
            reason = 'and dual licensing is not supported';
    }
    catch {
        // Report the declaration as unsupported, even when SPDX parsing fails.
    }
    return issue(packageInfo, `Your module is published under ${displayDeclaration(declaration)}, ${reason}. ${supportedProjectLicenseAdvice()} Talk to us if you have a reason to use a different license.`);
}
...
function createProjectIssues(packageInfo) {
    return [createDistributionLicenseIssue(packageInfo), createSourceLicenseIssue(packageInfo)].filter((projectIssue) => projectIssue !== undefined);
}
export function createLicensePolicyIssues(inventory) {
    ...
    for (const packageInfo of [...inventory.packages].sort(packageSort)) {
        const declaration = normalizedDeclaration(packageInfo);
        const identity = packageInfo.kind === 'project'
            ? 'project'
        ...
        if (!parsed || !parsed.strict) {
            ...
            if (corrected)
                parsed = parseDeclaration(corrected) ?? parsed;
        }
        if (!parsed) {
            result.push(issue(packageInfo, `Dependency ${packageLabel(packageInfo)} has unparseable license declaration ${displayDeclaration(declaration)}.`));
            continue;
        }
        ...
        if (evaluate(parsed.node, externalLicenses).allowed) {
            result.push(issue(packageInfo, `Dependency ${packageLabel(packageInfo)} has license declaration ${displayDeclaration(declaration)} which may only be shipped as an external dependency, not built into the bundle. Add it to the externals in build-config.cjs so it is installed alongside the module and loaded at runtime.`));
            continue;
        }
        if (!parsed.strict) {
            // Reaching here means no branch we could recognise was allowed, so the unrecognised ones are the problem
            result.push(issue(packageInfo, `Dependency ${packageLabel(packageInfo)} has license declaration ${displayDeclaration(declaration)} which is not a valid SPDX identifier, so it cannot be checked against the ${projectLicense} license policy. Ask the package author to declare a specific SPDX license.`));
            continue;
        }
        result.push(issue(packageInfo, `Dependency ${packageLabel(packageInfo)} has license declaration ${displayDeclaration(declaration)} which is not compatible with the ${projectLicense} license policy.${ambiguitySuffix(evaluation)}`));
    }
    return result;
}
export function enforceLicensePolicy(inventory, options = {}) {
    ...
    for (const policyIssue of issues)
        stderr.write(`${prefix}: ${policyIssue.message}\n`);
    if (options.ignoreLicenseRules) {
        stderr.write(`License validation ignored ${issues.length} error${issues.length === 1 ? '' : 's'} because --ignore-license-rules was provided.\n`);
        return;
    }
