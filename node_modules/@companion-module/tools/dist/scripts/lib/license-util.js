...
export function normalizeRepositoryUrl(packageJson) {
    ...
    if (url) {
        ...
        if (/^[\w.-]+\/[\w.-]+$/.test(url))
            url = `${SHORTHAND_REPOSITORY_HOSTS.github}${url}`;
    ...
    if (!/^https?:\/\//.test(url))
        url = typeof packageJson.homepage === 'string' ? packageJson.homepage.trim() : '';
...
function asLicenseString(license) {
    ...
    if (license && typeof license === 'object')
        return asLicenseString(license.type);
    return undefined;
}
...
function declaredLicenseOf(packageJson) {
    return (asLicenseString(packageJson.license) ??
...
async function findPackageRoot(inputPath, moduleDir) {
    const nodeModulesDir = path.join(moduleDir, 'node_modules');
    if (!inputPath.startsWith(`${nodeModulesDir}${path.sep}`))
        return undefined;
...
function getContributingInputs(metafile) {
...
export async function collectInstalledPackages(nodeModulesDir) {
    ...
    async function scan(currentNodeModulesDir) {
        ...
        for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
            if (entry.name === '.bin' || entry.isSymbolicLink())
                continue;
    ...
    async function scanScopedPackages(scopeDir) {
        ...
        for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
            if (entry.isDirectory() && !entry.isSymbolicLink())
                await scanPackage(path.join(scopeDir, entry.name));
        }
    }
...
async function readLegalText(filePath, packageRoot, role) {
    ...
    if (!fileStat.isFile() || fileStat.size > MAX_LEGAL_FILE_SIZE)
        return undefined;
...
export async function collectPackageLegalMaterial(packageInfo) {
    ...
    for (const entry of await readdir(packageInfo.packageRoot, { withFileTypes: true })) {
        ...
        if (role)
            candidateFiles.set(entry.name, role);
    }
    ...
    for (const [filename, role] of [...candidateFiles.entries()].sort(([a], [b]) => a.localeCompare(b))) {
        try {
            ...
            if (legalText)
                legalTexts.push(legalText);
            else
                diagnostics.push(`Ignoring unreadable, binary, or oversized legal file: ${filename}`);
        }
        catch {
    ...
    for (const legalText of legalTexts) {
        const key = legalText.role === 'source-comment'
...
function renderLegalFile(inventory, roles) {
    ...
    if (!groupedTexts.size)
        return undefined;
    const sections = [...groupedTexts.values()].sort((a, b) => comparePackages([...a.packages.values()].sort(comparePackages)[0], [...b.packages.values()].sort(comparePackages)[0]));
...
export async function createLegalInventory(packages) {
    ...
    for (const packageInfo of packages) {
        ...
        for (const sourcePath of packageInfo.contributingPaths)
            existing.contributingPaths.add(sourcePath);
        if (kindPriority[packageInfo.kind] < kindPriority[existing.kind])
            existing.kind = packageInfo.kind;
    }
    const diagnostics = [];
    const legalRecords = [];
...
export async function analyzeShippedLegalInventory(moduleDir) {
    ...
    const result = await esbuild.build(createEsbuildOptions(definition, {
...
export async function collectMetafilePackages(moduleDir, metafile) {
    ...
    for (const input of getContributingInputs(metafile)) {
        ...
        const kind = packageRoot === resolvedModuleDir ? 'project' : 'bundled';
