...
import { buildUtilityActions, type UtilityActionsSchema } from './actions/utility.js'
...
export function UpdateActions(self: ModuleInstance): void {
	self.setActionDefinitions({
		...
		client_reconnect_sta: {
			...
			description:
				"Press this to force a currently connected device to drop and immediately reconnect to Wi-Fi, useful when a device seems 'stuck' or is having connectivity trouble mid-event. This only affects the device's current session; it does not block or unblock it.",
			options: [
		...
		device_disable_ap: {
			...
			options: [
				// unifi-api-ts parameter "ap_id" (required)
				{
					...
					label: 'AP ID',
					tooltip:
						'Internal device ID of the access point (the long 24-character code), not its name — copy it from the List Devices output.',
					default: '',
				},
		...
		device_led_override: {
			...
			options: [
				// unifi-api-ts parameter "device_id" (required)
				{
					...
					label: 'Device ID',
					tooltip:
						"Internal device ID (the 24-character code), not the device's name — copy it from the List Devices output.",
					default: '',
				},
		...
		device_restart_device: {
			...
			options: [
				// unifi-api-ts parameter "macs" (required) — a string | string[], entered here as a comma-separated list
				{
					...
					default: '',
					tooltip:
						'MAC address of the device (or several, separated by commas) to restart, e.g. aa:bb:cc:dd:ee:ff (comma-separated list).',
				},
		...
		network_spectrum_scan: {
			...
			options: [
				// unifi-api-ts parameter "mac" (required)
				{
					...
					label: 'MAC',
					tooltip:
						"The MAC address of the access point to scan, in the format aa:bb:cc:dd:ee:ff — found on the AP's details page in the UniFi UI.",
					default: '',
				},
			],
			callback: async (event) => {
		...
		network_spectrum_scan_state: {
			...
			description:
				"Press this to retrieve the results of a spectrum scan you previously started on an access point, showing interference and channel usage details. Run the 'spectrum scan' action first and wait roughly 30 seconds before pressing this to get meaningful results.",
			options: [
				// unifi-api-ts parameter "mac" (required)
				{
					id: 'mac',
					...
					label: 'MAC',
					tooltip:
						'The MAC address of the access point whose scan results you want, in the format aa:bb:cc:dd:ee:ff — must match the AP you started the scan on.',
					default: '',
				},
			],
			callback: async (event) => {
		...
		stats_archive_alarm: {
			...
			description:
				"Press this to dismiss (archive) alarms in the controller so they no longer show as active. Leave the field blank to clear all current alarms at once, or enter one alarm's ID to clear just that one; use this after you've already reviewed the alarm and don't need it flagged anymore.",
			options: [
		...
		stats_cmd_stat: {
			...
			description:
				'Press this to run a statistics-related system command on the controller. The only supported command permanently clears all Deep Packet Inspection traffic history and starts fresh, so only press this if you specifically want to wipe that data.',
			options: [
				// unifi-api-ts parameter "command" (required)
				{
					...
					label: 'Command',
					tooltip:
						'The command to run. Currently only reset-dpi is supported, which permanently clears DPI (Deep Packet Inspection) traffic statistics. Type it exactly as reset-dpi.',
					default: '',
				},
			],
			callback: async (event) => {
