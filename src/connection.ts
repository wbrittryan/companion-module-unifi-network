...
import { resolvePort, type ModuleConfig, type ModuleSecrets } from './config.js'
...
// the parts the helpers below actually use.
...
function getAxiosClient(unifi: UniFiClient): AxiosLikeClient {
	return (unifi.getHttpClient() as unknown as { client: AxiosLikeClient }).client
}
...
function attachApiKey(unifi: UniFiClient, apiKey: string): void {
...
async function connectWithUserPass(config: ModuleConfig, secrets: ModuleSecrets): Promise<UniFiClient> {
...
async function connectWithApiKey(
	...
	apiKey: string,
	config: ModuleConfig,
	describeError: (statusCode: number | undefined) => string | undefined,
): Promise<UniFiClient> {
	const unifi = new UniFiClient({
		baseUrl,
...
export async function createUnifiClient(config: ModuleConfig, secrets: ModuleSecrets): Promise<UniFiClient> {
	...
	switch (authMethod) {
		...
		case 'networkApiKey': {
			...
			if (config.consoleType === 'selfHosted') {
				throw new Error(
					'Network API keys only work on UniFi OS consoles, not self-hosted Network Controllers. Switch the ' +
			...
			return connectWithApiKey(
				`https://${config.host}:${resolvePort(config)}/proxy/network`,
				apiKey,
				config,
				(status) => {
		...
		case 'siteManagerApiKey': {
			...
			return connectWithApiKey(
				`${SITE_MANAGER_BASE_URL}/v1/connector/consoles/${encodeURIComponent(consoleId)}/network`,
				apiKey,
				{ ...config, verifySsl: true },
				(status) => {
					if (status === 401 || status === 403) {
						return (
							'Site Manager rejected the API key. Check it is a Site Manager key from unifi.ui.com (not a ' +
