...
// Auto-generated from unifi-api-ts's SecurityAPI class: every public method it exposes becomes one
// Companion action below, named "Security: <action>". Do not hand-edit call sites here without also
// checking unifi-api-ts's SecurityAPI.d.ts, since the argument order must match it exactly.

// The shape of each action's options, so Companion's editor UI and the callback below stay in sync.
export type SecurityActionsSchema = {
	// Security: Create Firewall Group — options Companion stores for this action's button
	...
	// Security: List Firewall Groups — options Companion stores for this action's button
	security_list_firewallgroups: {
		options: Record<string, never>
	}
	// Security: List Firewall Rules — options Companion stores for this action's button
	security_list_firewallrules: {
		options: Record<string, never>
	}
	// Security: Delete Firewall Group — options Companion stores for this action's button
	security_delete_firewallgroup: {
		options: {
			group_id: string
		}
	}
	// Security: Edit Firewall Group — options Companion stores for this action's button
	...
	// Security: Create RADIUS Account — options Companion stores for this action's button
	...
	// Security: List RADIUS Accounts — options Companion stores for this action's button
	security_list_radius_accounts: {
		options: Record<string, never>
	}
	// Security: List RADIUS Profiles — options Companion stores for this action's button
	security_list_radius_profiles: {
		options: Record<string, never>
	}
	// Security: Delete RADIUS Account — options Companion stores for this action's button
	security_delete_radius_account: {
		options: {
			account_id: string
		}
	}
	// Security: Update RADIUS Account — options Companion stores for this action's button
	...
	// Security: Update IPS/IDS Settings — options Companion stores for this action's button
	security_set_ips_settings_base: {
		options: {
			payload: string
		}
	}
	// Security: Set WLAN MAC Filter — options Companion stores for this action's button
	...
	// Security: Update Guest Login Settings — options Companion stores for this action's button
	...
	// Security: Update Guest Login Settings Base — options Companion stores for this action's button
	...
	// Security: Update Site Guest Access — options Companion stores for this action's button
	...
	// Security: Update Site SNMP — options Companion stores for this action's button
...
export function buildSecurityActions(self: ModuleInstance): CompanionActionDefinitions<SecurityActionsSchema> {
	return {
		...
		security_create_firewallgroup: {
			...
			callback: async (event) => {
				...
				try {
					...
					const group_typeValue = event.options.group_type as 'address-group' | 'ipv6-address-group' | 'port-group'
				...
				try {
					...
					const result = await unifi
						...
						.create_radius_account(
							event.options.name,
							event.options.x_password,
							event.options.tunnel_type,
							event.options.tunnel_medium_type,
							event.options.vlan,
							self.config.site,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
		...
		security_set_wlan_mac_filter: {
			...
			callback: async (event) => {
				...
				try {
					// "site" is taken from the connection's configured Site (see the module's Settings tab), not a button field
					// Narrow the dropdown's plain string back to the exact literal type unifi-api-ts expects
					const mac_filter_policyValue = event.options.mac_filter_policy as 'allow' | 'deny'
					...
					const result = await unifi
						.getSecurityAPI()
						.set_wlan_mac_filter(
							event.options.wlan_id,
							mac_filter_policyValue,
							event.options.mac_filter_enabled,
							macsList,
							self.config.site,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
				...
				try {
					...
					const result = await unifi
						...
						.set_guestlogin_settings(
							event.options.portal_enabled,
							event.options.portal_customized,
							event.options.redirect_enabled,
							event.options.redirect_url,
							event.options.x_password,
							event.options.expire_number,
							event.options.expire_unit,
							event.options.section_id,
							self.config.site,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
