...
// Auto-generated from unifi-api-ts's DeviceManagementAPI class: every public method it exposes becomes one
// Companion action below, named "Devices: <action>". Do not hand-edit call sites here without also
// checking unifi-api-ts's DeviceManagementAPI.d.ts, since the argument order must match it exactly.

// The shape of each action's options, so Companion's editor UI and the callback below stay in sync.
export type DeviceActionsSchema = {
	// Devices: List Devices — options Companion stores for this action's button
	device_list_devices: {
		options: {
			macs: string
		}
	}
	// Devices: Adopt Device — options Companion stores for this action's button
	device_adopt_device: {
		options: {
			macs: string
		}
	}
	// Devices: Advanced Adopt Device — options Companion stores for this action's button
	...
	// Devices: Cancel Device Migration — options Companion stores for this action's button
	device_cancel_migrate_device: {
		options: {
			macs: string
		}
	}
	// Devices: Cancel Rolling Upgrade — options Companion stores for this action's button
	device_cancel_rolling_upgrade: {
		options: Record<string, never>
	}
	// Devices: Check Firmware Update — options Companion stores for this action's button
	device_check_firmware_update: {
		options: Record<string, never>
	}
	// Devices: Delete Device — options Companion stores for this action's button
	device_delete_device: {
		options: {
			mac: string
		}
	}
	// Devices: Disable Access Point — options Companion stores for this action's button
	device_disable_ap: {
		options: {
			ap_id: string
			disable: boolean
	...
	// Devices: Force Provision — options Companion stores for this action's button
	device_force_provision: {
		options: {
			macs: string
		}
	}
	// Devices: Override LED — options Companion stores for this action's button
	device_led_override: {
		options: {
			device_id: string
	...
	// Devices: List AP Groups — options Companion stores for this action's button
	device_list_apgroups: {
		options: Record<string, never>
	}
	// Devices: List Access Points — options Companion stores for this action's button
	device_list_aps: {
		options: {
			mac: string
		}
	}
	// Devices: List Device Name Mappings — options Companion stores for this action's button
	device_list_device_name_mappings: {
		options: Record<string, never>
	}
	// Devices: List Device States — options Companion stores for this action's button
	device_list_device_states: {
		options: Record<string, never>
	}
	// Devices: List Devices (Basic) — options Companion stores for this action's button
	device_list_devices_basic: {
		options: {
			device_mac: string
		}
	}
	// Devices: List Firmware — options Companion stores for this action's button
	device_list_firmware: {
		options: {
			device_type: string
		}
	}
	// Devices: List Device Models — options Companion stores for this action's button
	device_list_models: {
		options: Record<string, never>
	}
	// Devices: Locate Access Point — options Companion stores for this action's button
	device_locate_ap: {
		options: {
			mac: string
			enable: boolean
	...
	// Devices: Migrate Device — options Companion stores for this action's button
	...
	// Devices: Power Cycle Switch Port — options Companion stores for this action's button
	...
	// Devices: Reboot Cloud Key — options Companion stores for this action's button
	device_reboot_cloudkey: {
		options: Record<string, never>
	}
	// Devices: Rename Access Point — options Companion stores for this action's button
	device_rename_ap: {
		options: {
			ap_id: string
			ap_name: string
	...
	// Devices: Restart Device — options Companion stores for this action's button
	...
	// Devices: Set AP Radio Settings — options Companion stores for this action's button
	...
	// Devices: Set AP WLAN Group — options Companion stores for this action's button
	...
	// Devices: Set Device Settings — options Companion stores for this action's button
	...
	// Devices: Set Element Adoption — options Companion stores for this action's button
	device_set_element_adoption: {
		options: {
			enable: boolean
		}
	}
	// Devices: Start Rolling Upgrade — options Companion stores for this action's button
	device_start_rolling_upgrade: {
		options: {
			payload: string
		}
	}
	// Devices: Upgrade All Devices — options Companion stores for this action's button
	device_upgrade_all_devices: {
		options: {
			type: string
		}
	}
	// Devices: Upgrade Device — options Companion stores for this action's button
	device_upgrade_device: {
		options: {
			mac: string
		}
	}
	// Devices: Upgrade Device (External Firmware) — options Companion stores for this action's button
	...
	// Devices: Create AP Group — options Companion stores for this action's button
	...
	// Devices: Delete AP Group — options Companion stores for this action's button
	device_delete_apgroup: {
		options: {
			group_id: string
		}
	}
	// Devices: Edit AP Group — options Companion stores for this action's button
	device_edit_apgroup: {
		options: {
			group_id: string
			group_name: string
...
export function buildDeviceActions(self: ModuleInstance): CompanionActionDefinitions<DeviceActionsSchema> {
	return {
		...
		device_list_devices: {
			...
			description:
				'Retrieves information about UniFi devices (Access Points, Switches, Gateways, etc.) in the network, optionally filtered by MAC address(es).',
			options: [
				...
				try {
					...
					const result = await unifi
						...
						.advanced_adopt_device(
							event.options.mac,
							event.options.ip,
							event.options.username,
							event.options.password,
							event.options.url,
							event.options.port,
							event.options.ssh_key_verify,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
		...
		device_disable_ap: {
			...
			description:
				'Disables or enables an Access Point device; when disabled the AP stops broadcasting wireless networks.',
			options: [
		...
		device_force_provision: {
			...
			description:
				'Forces provisioning of one or more UniFi devices, triggering them to re-download their configuration.',
			options: [
				// unifi-api-ts parameter "macs" (required) — a raw JSON payload, typed here as text
				{
					id: 'macs',
		...
		device_led_override: {
			...
			description:
				'Sets the LED override mode (on, off, or default) for a device (no JSDoc description present in source).',
			options: [
			...
			callback: async (event) => {
				...
				try {
					...
					const override_modeValue = event.options.override_mode as 'off' | 'on' | 'default'
		...
		device_list_device_states: {
			...
			description:
				'Retrieves current state information for all devices, including connection status and operational state.',
			options: [],
			callback: async () => {
				// Every action bails out early if the module isn't currently connected to the console
				const unifi = self.unifi
		...
		device_list_devices_basic: {
			...
			description:
				'Retrieves basic device information with a reduced data payload, faster than the full device listing.',
			options: [
		...
		device_list_models: {
			...
			description:
				'Retrieves information about supported device models, including names, capabilities, and specifications.',
			options: [],
			callback: async () => {
				// Every action bails out early if the module isn't currently connected to the console
				const unifi = self.unifi
		...
		device_locate_ap: {
			...
			description:
				'Enables or disables the locate LED feature on an Access Point to help physically locate the device.',
			options: [
				// unifi-api-ts parameter "mac" (required)
				{
					id: 'mac',
		...
		device_migrate_device: {
			...
			description:
				"Migrates one or more UniFi devices to a different controller using the target controller's inform URL.",
			options: [
				// unifi-api-ts parameter "macs" (required) — a raw JSON payload, typed here as text
				{
					id: 'macs',
		...
		device_power_cycle_switch_port: {
			...
			description:
				'Power cycles a specific port on a UniFi switch, turning it off and back on to reset connected devices.',
			options: [
				// unifi-api-ts parameter "mac" (required)
				{
					id: 'mac',
		...
		device_restart_device: {
			...
			callback: async (event) => {
				...
				try {
					...
					const macsPayload = JSON.parse(event.options.macs)
					// Narrow the dropdown's plain string back to the exact literal type unifi-api-ts expects
					const reboot_typeValue = event.options.reboot_type as 'soft' | 'hard'
				...
				try {
					...
					const result = await unifi
						...
						.set_ap_radiosettings(
							...
							event.options.ht,
							event.options.tx_power_mode,
							event.options.tx_power,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
		...
		device_set_ap_wlangroup: {
			...
			description:
				"Assigns an Access Point's radio to a specific WLAN group, controlling which networks broadcast on which radio.",
			options: [
			...
			callback: async (event) => {
				...
				try {
					// Narrow the dropdown's plain string back to the exact literal type unifi-api-ts expects
					const type_idValue = event.options.type_id as 'ng' | 'na'
		...
		device_set_device_settings_base: {
			...
			description:
				"Updates a device's settings using a custom configuration payload; a low-level method for advanced device configuration.",
			options: [
				// unifi-api-ts parameter "device_id" (required)
				{
					id: 'device_id',
		...
		device_start_rolling_upgrade: {
			...
			description:
				'Initiates a rolling firmware upgrade for specified device types, upgrading them automatically in sequence.',
			options: [
		...
		device_upgrade_device_external: {
			...
			description:
				'Upgrades one or more devices using firmware retrieved from an external URL, allowing custom or beta firmware.',
			options: [
		...
		device_delete_apgroup: {
			...
			description:
				'Permanently removes an Access Point group from the controller; devices in it move to the default group.',
			options: [
				// unifi-api-ts parameter "group_id" (required)
				{
					id: 'group_id',
