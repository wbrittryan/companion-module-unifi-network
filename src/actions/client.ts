...
// Auto-generated from unifi-api-ts's ClientManagementAPI class: every public method it exposes becomes one
// Companion action below, named "Clients: <action>". Do not hand-edit call sites here without also
// checking unifi-api-ts's ClientManagementAPI.d.ts, since the argument order must match it exactly.

// The shape of each action's options, so Companion's editor UI and the callback below stay in sync.
export type ClientActionsSchema = {
	// Clients: List Users — options Companion stores for this action's button
	client_list_users: {
		options: Record<string, never>
	}
	// Clients: List Active Clients — options Companion stores for this action's button
	client_list_active_clients: {
		options: {
			include_traffic_usage: boolean
			include_unifi_devices: boolean
		}
	}
	// Clients: List Clients — options Companion stores for this action's button
	client_list_clients: {
		options: {
			mac: string
		}
	}
	// Clients: List Client History — options Companion stores for this action's button
	...
	// Clients: Get Client Details — options Companion stores for this action's button
	client_stat_client: {
		options: {
			mac: string
		}
	}
	// Clients: Block Client — options Companion stores for this action's button
	client_block_sta: {
		options: {
			mac: string
		}
	}
	// Clients: Unblock Client — options Companion stores for this action's button
	client_unblock_sta: {
		options: {
			mac: string
		}
	}
	// Clients: Reconnect Client — options Companion stores for this action's button
	client_reconnect_sta: {
		options: {
			mac: string
		}
	}
	// Clients: Forget Client — options Companion stores for this action's button
	client_forget_sta: {
		options: {
			macs: string
		}
	}
	// Clients: Edit Client Fixed IP — options Companion stores for this action's button
	...
	// Clients: Edit Client Name — options Companion stores for this action's button
	...
	// Clients: Set Client Name — options Companion stores for this action's button
	client_set_sta_name: {
		options: {
			user_id: string
			name: string
	...
	// Clients: Set Client Note — options Companion stores for this action's button
	client_set_sta_note: {
		options: {
			user_id: string
			note: string
	...
	// Clients: Authorize Guest — options Companion stores for this action's button
	...
	// Clients: Unauthorize Guest — options Companion stores for this action's button
	client_unauthorize_guest: {
		options: {
			mac: string
		}
	}
	// Clients: Extend Guest Validity — options Companion stores for this action's button
	client_extend_guest_validity: {
		options: {
			guest_id: string
		}
	}
	// Clients: List Guests — options Companion stores for this action's button
	client_list_guests: {
		options: {
			within: number
		}
	}
	// Clients: Get Sessions — options Companion stores for this action's button
	...
	// Clients: Get Latest Station Sessions — options Companion stores for this action's button
	...
	// Clients: Create Tag — options Companion stores for this action's button
	client_create_tag: {
		options: {
			name: string
			macs: string
	...
	// Clients: Delete Tag — options Companion stores for this action's button
	client_delete_tag: {
		options: {
			tag_id: string
		}
	}
	// Clients: Get Tag — options Companion stores for this action's button
	client_get_tag: {
		options: {
			tag_id: string
		}
	}
	// Clients: List Tags — options Companion stores for this action's button
	client_list_tags: {
		options: Record<string, never>
	}
	// Clients: Set Tagged Devices — options Companion stores for this action's button
	...
	// Clients: Create Voucher — options Companion stores for this action's button
	...
	// Clients: Revoke Voucher — options Companion stores for this action's button
	client_revoke_voucher: {
		options: {
			voucher_id: string
		}
	}
	// Clients: Create Hotspot Operator — options Companion stores for this action's button
	...
	// Clients: List Hotspot Operators — options Companion stores for this action's button
	client_list_hotspotop: {
		options: Record<string, never>
	}
}

export function buildClientActions(self: ModuleInstance): CompanionActionDefinitions<ClientActionsSchema> {
	return {
		...
		client_list_users: {
			...
			description:
				'Retrieves a list of all client devices, both currently connected and previously connected, that have connected to the UniFi network.',
			options: [],
			callback: async () => {
		...
		client_list_active_clients: {
			...
			description:
				'Lists currently active client devices, optionally including traffic usage data and UniFi infrastructure devices in the results.',
			options: [
			...
			callback: async (event) => {
				...
				try {
					...
					const result = await unifi
						...
						.list_active_clients(
							self.config.site,
							event.options.include_traffic_usage,
							event.options.include_unifi_devices,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
		...
		client_block_sta: {
			...
			description:
				'Blocks network access for a specific client device by MAC address until it is explicitly unblocked.',
			options: [
				// unifi-api-ts parameter "mac" (required)
				{
					id: 'mac',
			...
			callback: async (event) => {
				...
				try {
					...
					const result = await unifi
						...
						.edit_client_fixedip(
							self.config.site,
							event.options.client_id,
							event.options.use_fixedip,
							event.options.network_id,
							event.options.fixed_ip,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
		...
		client_authorize_guest: {
			...
			description:
				'Grants a guest device network access for a specified duration, with optional bandwidth limits, a data quota, and AP restriction.',
			options: [
				// unifi-api-ts parameter "mac" (required)
				{
					id: 'mac',
				...
				try {
					...
					const result = await unifi
						...
						.authorize_guest(
							...
							event.options.up,
							event.options.down,
							event.options.megabytes,
							event.options.ap_mac,
						)
					// Log the raw result at debug level so it shows up in Companion's log if you need to troubleshoot
		...
		client_stat_sessions: {
			...
			callback: async (event) => {
				...
				try {
					...
					const typeValue = event.options.type as 'all' | 'guest' | 'user'
		...
		client_create_tag: {
			...
			description:
				'Creates a new tag for organizing client devices, optionally assigning an initial set of devices to it.',
			options: [
				// unifi-api-ts parameter "name" (required)
				{
					id: 'name',
		...
		client_create_voucher: {
			...
			description:
				'Creates one or more guest access vouchers with a specified duration, usage quota, and optional bandwidth or data limits.',
			options: [
				// unifi-api-ts parameter "minutes" (required)
				{
					id: 'minutes',
